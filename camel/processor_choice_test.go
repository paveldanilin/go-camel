...
func TestChoiceProcessor(t *testing.T) {
	choice := newChoiceProcessor("test val").
		addWhen(mustSimpleExpression("header.val > 5"), newSetBodyProcessor("", newConstExpression(555))).
		addWhen(mustSimpleExpression("header.val < 5"), newSetBodyProcessor("", newConstExpression(777)))
